...
private extension AuthController{
    ...
    func accessToken(for user: LoginUser, on connnection: DatabaseConnectable) throws -> Future<AccessToken> {
        return try AccessToken(userID: user.userID ?? "").save(on: connnection)
    }
    
    func refreshToken(for user: LoginUser, on connnection: DatabaseConnectable) throws -> Future<RefreshToken> {
        return try RefreshToken(userID: user.userID ?? "").save(on: connnection)
    }
